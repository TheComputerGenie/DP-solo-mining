...
```javascript
{
    "coin": "DP.json",                              // the file in 'coins' folder that defines current used coin
    "address": "RHznkY6Q1rsQKpwPDpbFpTWnDKDSZhwmoB",    // the transparent address you want reward to go to
    "daemons": [                    // any number of access allowed daemons -- for most users this will only be 1 local daemon
        {
            ...
            "port": 28388,          // open port -- can be found with `getinfo` call
    ...
    "p2p": {
        ...
        "port": 28387,                  // open port -- can be found with `getinfo` call
    ...
    "ports": {          // This is where you set ports and difficulty levels for miners to connect
        "3883": {       // Port to open for miners
            "diff": 15   // difficulty for that port -- 1 Difficulty is actually 8192, 0.125 Difficulty is actually 1024 
...
```javascript
{
    "name": "DigitalPrice",           // The name of the coin
    "symbol": "DP",            // The coin's ticker symbol
    "peerMagic": "afcf0e7e",    // easiest way to find this is run daemon -- magic.7e0ecfaf becomes afcf0e7e
